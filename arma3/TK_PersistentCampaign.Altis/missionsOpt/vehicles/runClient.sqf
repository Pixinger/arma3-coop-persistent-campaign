...
_currentTask = player createSimpleTask [format["Fahrzeuge zerstören (%1)", gettext (configFile >> "CfgVehicles" >> _vehicleClassname >> "displayName")]];
_currentTask setSimpleTaskDescription [format["Unser Geheimdienst hat eine Fahrzeuggruppe ermittelt. Vernichten sie diese Fahrzeuge (Typ: %1)", gettext (configFile >> "CfgVehicles" >> _vehicleClassname >> "displayName")], format["Fahrzeuge zerstören (%1)", gettext (configFile >> "CfgVehicles" >> _vehicleClassname >> "displayName")], ""];
...
/*------------------------------------*/
player sidechat "apc: waiting for mission status";
waitUntil { str(pvehPixZones_MissionStatus) != "[]" };
player sidechat "apc: mission status received";
