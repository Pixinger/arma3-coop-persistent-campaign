...
private _markerName = format["markerUField%1_%2", _zoneIndex, floor(random 999999)];
	...
	switch (_state) do 
	{
			...
			while { true } do
			{
				...
				{ 
					#ifndef NO_MARKERS
					{
						...
						_mn setMarkerText format["F|%1", _markerCounter];
				...
				{
					private _mn = _markerNames select _foreachindex;
					_mn setMarkerText format["F|EX|%1|%2|%3", _markerCounter, _foreachindex, _zoneIndex];
					_mn setMarkerPos (getPos _x);
				} foreach units _group;
				#endif
		...
		{ 
			while { true } do
			{
				...
				{
					private _mn = _markerNames select _foreachindex;
					_mn setMarkerText format["F|RD|%1|%2|%3", _markerCounter, _foreachindex, _zoneIndex];
					_mn setMarkerPos (getPos _x);
				} foreach units _group;
				#endif
...
-				{
					private _mn = _markerNames select _foreachindex;
					_mn setMarkerText format["F|FL|%1|%2|%3-", _markerCounter, _foreachindex, _zoneIndex];
					_mn setMarkerPos (getPos _x);
				} foreach units _group;
				#endif
		...
		{ 
			...
			{
				private _mn = _markerNames select _foreachindex;
				_mn setMarkerText format["F|EXIT|%1|%2|%3-", _markerCounter, _foreachindex, _zoneIndex];
				_mn setMarkerPos (getPos _x);
			} foreach units _group;
			#endif
			_run = false; // Exit
