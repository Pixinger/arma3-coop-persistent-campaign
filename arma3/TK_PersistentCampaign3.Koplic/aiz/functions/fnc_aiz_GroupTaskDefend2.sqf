...
{
	private _unitPosition = _position findEmptyPosition [[5, floor(_guardRadius / 2)] call BIS_fnc_randomInt, _guardRadius, "SoldierWB"];
	if ((count _unitPosition) == 0) then { _unitPosition = _position findEmptyPosition [5, _guardRadius, "SoldierWB"] };
	if ((count _unitPosition) > 0) then 
	{ 
		//[_x, _unitPosition, true] spawn fnc_aiz_DoMoveEx;
		doStop _x;
		_x setPos (_unitPosition);
