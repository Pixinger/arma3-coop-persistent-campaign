...
if (pixDebug) then { player sidechat format["INFO: fn_pixZones_UpdateMarkerColor: pvehPixZones_ZoneStatus=%1", pvehPixZones_ZoneStatus]; };
...
for "_i" from 0 to (pixZones_ZoneCount-1) do
{
	if (_i < count pvehPixZones_ZoneStatus) then
	{
		...
		{ 
			...
			case 0: 
			{ 
				if ([_i] call fn_pixZones_CanEngageZone) then
				{
					/* Angreifbar */
