if (isServer) then
{
	...
	if (_result == "OK") then
	{
		...
		if (_result != "OK") then
		{
			...
			player sidechat format["ERROR: fn_pixZones_DbWriteNext failed: %1", _result];
		};		
		...
		if (_result == "OK") then
		{
			...
			{	
				...
				player sidechat "INFO: pixZones: Db saved";
			}
			else
			{
				diag_log format["ERROR: fn_pixZones_DbEndSave=>fn_pixZones_DbIsThreadFinished failed: %1", _result];
				player sidechat format["ERROR: fn_pixZones_DbEndSave=>fn_pixZones_DbIsThreadFinished failed: %1", _result];
			};
		}
		else
		{
			diag_log format["ERROR: fn_pixZones_DbEndSave failed: %1", _result];
			player sidechat format["ERROR: fn_pixZones_DbEndSave failed: %1", _result];
		};
	}
	else
	{
		diag_log format["ERROR: fn_pixZones_DbBeginSave failed: %1", _result];
		player sidechat format["ERROR: fn_pixZones_DbBeginSave failed: %1", _result];
	};
