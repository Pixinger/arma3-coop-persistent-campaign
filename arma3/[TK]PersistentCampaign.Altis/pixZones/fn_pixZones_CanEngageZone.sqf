/*
 Prüft ob eine bestimmte Zone von den eigenen Kräften angegriffen werden kann.
 Result: true wenn ein Angriff möglich ist, sonst false.
*/
...
if (_redZoneIndex < _zoneStatusCount) then /* pvehPixZones_ZoneStatus könnte == [] sein */
{
	...
	if ((pvehPixZones_ZoneStatus select _redZoneIndex) == 0) then
	{
		...
		private["_greenZones"];
		_greenZones = pixZones_ZoneEngagementConditions select _redZoneIndex;
