...
if (isServer) then
{
	...
	_townCivCount = 1;//DB		// Die Anzahl der Zivilisten in der Stadt (virtuell).
	private["_townRedCount"];
	_townRedCount = 0;//DB		// Die Anzahl der Feinde in der Stadt (virtuell).
...
diag_log format["_conversion=%1", _conversion];
...
diag_log format["_townStockWater=%1", _townStockPower];
...
diag_log format["_townCivCount=%1", _townCivCount];
		...
		// Injured simulieren
		// -----------------------------------
diag_log format["before _townInjuredCount=%1", _townInjuredCount];
		...
		if (_townInjuredCount < 0) then { _townInjuredCount = 0; };
diag_log format["after _townInjuredCount=%1", _townInjuredCount];
		...
		if ([_townCenter, _townRadius] call PC_fnc_IsAnyPlayerNear) then
		{
			...
			if (_redSOLL > _redActivesCount) then
			{
				...
				[_homes, pixTown_ConfigRedClassnames, _redActives, _count, east, "town\fsm\red.fsm", _townExternData1] call PC_fnc_TownHome_Units_Activate;
			};			
...
diag_log format["create civ: %1", _unit];
					};
				};
			};			
